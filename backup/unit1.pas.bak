...
var q,w,e,r,t,q1,w1,e1,r1,t1,a,b,c,aa,bb,cc,dd,m,n,y1,d,p3,q3,Af,Af1,Af2,Bt,Bt1,Bt2,Re,Im,F,x1,x2,x3,x4,z1,z2,d1,d2,b1,c1: extended;
begin
 ...
 begin if checkbox4.checked=False then
  begin if checkbox3.checked=false then
   begin if checkbox2.checked=false then
    edit6.Text:='X = 0' else
   begin
    if q=0 then Edit6.Text:='X = 0' else
    begin
    x1:=-w/q;
  ...
  begin
   if q=0 then
    begin
     if w=0 then Edit6.Text:='X = 0' else
    begin
    x1:=-e/w;
   ...
   begin
       ...
       begin
        ...
        Bt:=sqrt(-d)/2*q;
        if Af=1 then begin
         Edit6.Text:='X[1] = X[2] = 0;  X[3,4] = '+FloatToStr(Af)+' +/- i  (Complex)'; end
        else begin
         Edit6.Text:='X[1] = X[2] = 0;  X[3,4] = '+FloatToStr(Af)+' +/- '+FloatToStr(Bt)+'*i  (Complex)'; end
       end
              else
       begin
         x1:=(-w-sqrt(d))/(2*q);
         x2:=(-w+sqrt(d))/(2*q);
         if x1=x2
         then Edit6.Text:='X[1] = X[2] = 0;  X[3] = X[4] = '+FloatToStr(x1)
         else Edit6.Text:='X[1] = X[2] = 0;  X[3] = '+FloatToStr(x1)+';  X[4] = '+FloatToStr(x2);
       end;
     end;
   Edit12.Visible:=True;
  end;
        ...
        begin
         ...
         begin
          ...
          begin
           ...
           Edit6.Text:='X[1] = 0;  X[2] = X[3] = '+FloatToStr(x1)+';  X[4] = '+FloatToStr(x2);
          end
          else
          Edit6.Text:='X[1] = 0;  X[2] = '+FloatToStr(x1)+';  X[3] = '+FloatToStr(x2)+';  X[4] = '+FloatToStr(x3);
          if (x1=x2) and (x2=x3) then Edit12.Text:='Q = 0';
          if (x1=x2) then Edit12.Text:='Q = 0';
          if (x2=x3) then Edit12.Text:='Q = 0';
          if (x1=x3) then Edit12.Text:='Q = 0';
         end
         else Edit6.Text:='ERROR. Wtite me to inc8@me.com with mark "ERROR0001". Thanks';
         end;
        ...
        begin
         ...
         begin
          ...
          begin
           ...
           Edit6.Text:='X[1] = X[2] = '+FloatToStr(x1)+';  X[3] = '+FloatToStr(x2);
          end
          else
    ...
    begin
     ...
     begin
      ...
      d1:=b*b-4*c;
       if d1<0 then
       begin
        Af1:=-b/2{(2*q1*w1*e1*r1*t1)};
        Bt1:=sqrt(-d1)/2{(2*q1*w1*e1*r1*t1)};
       end
              else
       begin
         x1:=(-r-sqrt(d1))/2{(2*q1*w1*e1*r1*t1) };
         x2:=(-r+sqrt(d1))/2{(2*q1*w1*e1*r1*t1)};
       end;
    end;
    begin
      b:=aa/2+m;
      ...
      d2:=b*b-4*c;
       if d2<0 then
       begin
        Af2:=-b/2{(2*q1*w1*e1*r1*t1)};
        Bt2:=sqrt(-d2)/2{(2*q1*w1*e1*r1*t1)};
       end
              else
       begin
         x3:=(-r-sqrt(d2))/2{(2*q1*w1*e1*r1*t1)};
         x4:=(-r+sqrt(d2))/2{(2*q1*w1*e1*r1*t1)};
       end;
      end;
    if d1=d2 then begin
